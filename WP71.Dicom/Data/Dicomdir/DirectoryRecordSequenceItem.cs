...
namespace Dicom.Data
{
	...
	internal static class DirectoryRecordTypeDictionary
	{
		...
		static DirectoryRecordTypeDictionary()
		{
			...
			foreach (FieldInfo fi in infos)
			{
				...
				_nameList.Add(attrib.Name, (DirectoryRecordType)Enum.Parse(typeof(DirectoryRecordType), fi.Name, true));
				_typeList.Add((DirectoryRecordType)Enum.Parse(typeof(DirectoryRecordType), fi.Name, true), attrib.Name);
			}
		}
	...
	/// </summary>
    public class DirectoryRecordSequenceItem : DcmItemSequenceItem
	{
		...
		public DirectoryRecordType DirectoryRecordType
		{
			get
			{
                string recordType = base.Dataset.GetString(DicomTags.DirectoryRecordType, String.Empty);
		...
		public override string ToString()
		{
			...
			if (DirectoryRecordType == DirectoryRecordType.Series)
                toString = base.Dataset.GetString(DicomTags.SeriesInstanceUID, String.Empty);
			else if (DirectoryRecordType == DirectoryRecordType.Study)
                toString = base.Dataset.GetString(DicomTags.StudyInstanceUID, String.Empty);
			else if (DirectoryRecordType == DirectoryRecordType.Patient)
                toString = base.Dataset.GetValueString(DicomTags.PatientID) + " " + base.Dataset.GetValueString(DicomTags.PatientsName);
			else
                toString = base.Dataset.GetString(DicomTags.ReferencedSOPInstanceUIDInFile, String.Empty);
